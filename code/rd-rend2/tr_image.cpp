...
static void R_MipMapsRGB( byte *in, int inWidth, int inHeight)
{
	...
	outWidth = inWidth >> 1;
	outHeight = inHeight >> 1;
	temp = (byte *)Z_Malloc( outWidth * outHeight * 4, TAG_TEMP_WORKSPACE, qfalse );
...
static void RawImage_ScaleToPower2( byte **data, int *inout_width, int *inout_height, int *inout_scaled_width, int *inout_scaled_height, imgType_t type, int flags, byte **resampledBuffer)
{
	     ...
	     scaled_width < r_imageUpsampleMaxSize->integer && scaled_height < r_imageUpsampleMaxSize->integer)
	{
		...
		*data = *resampledBuffer;
		...
		{
			*resampledBuffer = (byte *)Z_Malloc( scaled_width * scaled_height * 4, TAG_TEMP_WORKSPACE, qfalse );
			ResampleTexture (*data, width, height, *resampledBuffer, scaled_width, scaled_height);
			*data = *resampledBuffer;
		}
		width = scaled_width;
	...
	qboolean lightMap, GLenum internalFormat, int *pUploadWidth, int *pUploadHeight)
{
	...
	scaledBuffer = (byte *)Z_Malloc( sizeof( unsigned ) * scaled_width * scaled_height, TAG_TEMP_WORKSPACE, qfalse );
...
image_t *R_CreateImage( const char *name, byte *pic, int width, int height, imgType_t type, int flags, int internalFormat ) {
	...
	image = tr.images[tr.numImages] = (image_t *)Z_Malloc( sizeof( image_t ), TAG_IMAGE_T, qfalse );
...
image_t	*R_FindImageFile( const char *name, imgType_t type, int flags )
{
	...
	if (r_normalMapping->integer && !(type == IMGTYPE_NORMAL) && (flags & IMGFLAG_PICMIP) && (flags & IMGFLAG_MIPMAP) && (flags & IMGFLAG_GENNORMALMAP))
	{
		...
		if (normalImage == NULL)
		{
			...
			normalWidth = width;
			normalHeight = height;
			normalPic = (byte *)Z_Malloc(width * height * 4, TAG_TEMP_WORKSPACE, qfalse);
