...
// 			mdxmWeight_t	*w;
 ...
 //			w = v->weights;
			...
			float fTotalWeight = 0.0f;
 			...
 			v++;// = (mdxmVertex_t *)&v->weights[surface->maxVertBoneWeights];
 		}
...
qboolean R_LoadMDXM( model_t *mod, void *buffer, const char *mod_name, qboolean &bAlreadyCached ) {
	...
	mod->data.glm = (mdxmData_t *)Z_Malloc (sizeof (mdxmData_t), TAG_HUNKALLOC, qfalse);
...
//
	...
	mod->data.glm->vboModels = (mdxmVBOModel_t *)Z_Malloc (sizeof (mdxmVBOModel_t) * mdxm->numLODs, TAG_HUNKALLOC, qtrue);
	for ( l = 0; l < mdxm->numLODs; l++ )
	{
		...
		int *baseVertexes = (int *)Z_Malloc (sizeof (int) * (mdxm->numSurfaces + 1), TAG_TEMP_WORKSPACE, qtrue);
		int *indexOffsets = (int *)Z_Malloc (sizeof (int) * mdxm->numSurfaces, TAG_TEMP_WORKSPACE, qtrue);

		vboModel->numVBOMeshes = mdxm->numSurfaces;
		vboModel->vboMeshes = (mdxmVBOMesh_t *)Z_Malloc (sizeof (mdxmVBOMesh_t) * mdxm->numSurfaces, TAG_HUNKALLOC, qtrue);
...
#ifdef USE_VERT_TANGENT_SPACE
		tangentsf = (vec3_t *)Z_Malloc (sizeof (vec3_t) * numVerts, TAG_TEMP_WORKSPACE, qfalse);
		bitangentsf = (vec3_t *)Z_Malloc (sizeof (vec3_t) * numVerts, TAG_TEMP_WORKSPACE, qfalse);
#endif
