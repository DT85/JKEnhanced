...
void G2_List_Model_Surfaces(const char *fileName)
{
	...
	mdxmData_t *mdxm = mod_m->data.glm;
	surf = (mdxmSurfHierarchy_t *) ( (byte *)mdxm->header + mdxm->header->ofsSurfHierarchy );
	mdxmSurface_t *surface = (mdxmSurface_t *)((byte *)mdxm->header + mdxm->header->ofsLODs + sizeof(mdxmLOD_t));

	for ( x = 0 ; x < mdxm->header->numSurfaces ; x++) 
	{
