...
model_t *R_AllocModel( void ) {
	...
	mod = (model_t *)Z_Malloc( sizeof( *tr.models[tr.numModels] ), TAG_MODEL, qfalse );
...
qboolean R_LoadMDXM_Server( model_t *mod, void *buffer, const char *mod_name, qboolean &bAlreadyCached ) {
	...
	mod->data.glm = (mdxmData_t *)Z_Malloc (sizeof (mdxmData_t), TAG_GHOUL2, qfalse);
...
//  Com_Memcpy(mod->md3[lod], buffer, LittleLong(md3Model->ofsEnd));
	...
	mdvModel->frames = frame = (mdvFrame_t *)Z_Malloc(sizeof(*frame) * md3Model->numFrames, TAG_MODEL_MD3, qtrue);
	...
	mdvModel->tags = tag = (mdvTag_t *)Z_Malloc(sizeof(*tag) * (md3Model->numTags * md3Model->numFrames), TAG_MODEL_MD3, qtrue);
	...
	mdvModel->tagNames = tagName = (mdvTagName_t *)Z_Malloc(sizeof(*tagName) * (md3Model->numTags), TAG_MODEL_MD3, qtrue);
	...
	mdvModel->surfaces = surf = (mdvSurface_t *)Z_Malloc(sizeof(*surf) * md3Model->numSurfaces, TAG_MODEL_MD3, qtrue);
	...
	for(i = 0; i < md3Model->numSurfaces; i++)
	{
		...
		surf->shaderIndexes = shaderIndex = (int *)Z_Malloc(sizeof(*shaderIndex) * md3Surf->numShaders, TAG_MODEL_MD3, qtrue);
		...
		surf->indexes = tri = (glIndex_t *)Z_Malloc(sizeof(*tri) * 3 * md3Surf->numTriangles, TAG_MODEL_MD3, qtrue);
		...
		surf->verts = v = (mdvVertex_t *)Z_Malloc(sizeof(*v) * (md3Surf->numVerts * md3Surf->numFrames), TAG_MODEL_MD3, qtrue);
		...
		// swap all the ST
		surf->st = st = (mdvSt_t *)Z_Malloc(sizeof(*st) * md3Surf->numVerts, TAG_MODEL_MD3, qtrue);
...

	{
		...
		mdvModel->vboSurfaces = (srfVBOMDVMesh_t *)Z_Malloc(sizeof(*mdvModel->vboSurfaces) * mdvModel->numSurfaces, TAG_MODEL_MD3, qtrue);
...
static qboolean R_LoadMDR( model_t *mod, void *buffer, int filesize, const char *mod_name ) 
{
	...
	mod->dataSize += size;
	mod->data.mdr = mdr = (mdrHeader_t*)Z_Malloc( size, TAG_MODEL_MD3, qtrue );
...
void R_LerpTag( orientation_t *tag, qhandle_t handle, int startFrame, int endFrame, 
	...
	if ( !model->data.mdv[0] )
	{
		if(model->type == MOD_MDR)
		{
			...
			R_IQMLerpTag( tag, (iqmData_t *)model->data.iqm,
					startFrame, endFrame,
					frac, tagName );
			return;
		} else {
