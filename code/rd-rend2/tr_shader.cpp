...
infoParm_t	infoParms[] = {
	// Game content Flags
	{ "nonsolid",		~CONTENTS_SOLID,					0,				CONTENTS_NONE },		// special hack to clear solid flag
	{ "nonopaque",		~CONTENTS_OPAQUE,					0,				CONTENTS_NONE },		// special hack to clear opaque flag
	...
	/* Game surface flags */
	{ "sky",			-1,									SURF_SKY,			CONTENTS_NONE },		// emit light from an environment map
	{ "slick",			-1,									SURF_SLICK,			CONTENTS_NONE },		// 

	{ "nodamage",		-1,									SURF_NODAMAGE,		CONTENTS_NONE },		// 
	{ "noimpact",		-1,									SURF_NOIMPACT,		CONTENTS_NONE },		// don't make impact explosions or marks
	{ "nomarks",		-1,									SURF_NOMARKS,		CONTENTS_NONE },		// don't make impact marks, but still explode
	{ "nodraw",			-1,									SURF_NODRAW,		CONTENTS_NONE },		// don't generate a drawsurface (or a lightmap)
	{ "nosteps",		-1,									SURF_NOSTEPS,		CONTENTS_NONE },		// 
	{ "nodlight",		-1,									SURF_NODLIGHT,		CONTENTS_NONE },		// don't ever add dynamic lights
	{ "metalsteps",		-1,									SURF_METALSTEPS,	CONTENTS_NONE },		// 
	{ "nomiscents",		-1,									SURF_NOMISCENTS,	CONTENTS_NONE },		// No misc ents on this surface
	{ "forcefield",		-1,									SURF_FORCEFIELD,	CONTENTS_NONE },		// 
	{ "forcesight",		-1,									SURF_FORCESIGHT,	CONTENTS_NONE },		// only visible with force sight
};
...
static shader_t *GeneratePermanentShader( void ) {
	...
	newShader = (shader_t *)Z_Malloc( sizeof( shader_t ), TAG_HUNKALLOC, qfalse );
	...
	*newShader = shader;
	...
	for ( i = 0 ; i < newShader->numUnfoggedPasses ; i++ ) {
		...
		newShader->stages[i] = (shaderStage_t *)Z_Malloc( sizeof( stages[i] ), TAG_HUNKALLOC, qfalse );
		*newShader->stages[i] = stages[i];

		for ( b = 0 ; b < NUM_TEXTURE_BUNDLES ; b++ ) {
			size = newShader->stages[i]->bundle[b].numTexMods * sizeof( texModInfo_t );
			newShader->stages[i]->bundle[b].texMods = (texModInfo_t *)Z_Malloc( size, TAG_HUNKALLOC, qfalse );
...
static void ScanAndLoadShaderFiles( void )
{
	...
	s_shaderText = (char *)Z_Malloc( sum + numShaderFiles*2, TAG_SHADERTEXT, qfalse );
	s_shaderText[ 0 ] = '\0';
	...
	hashMem = (char *)Z_Malloc( size * sizeof(char *), TAG_SHADERTEXT, qtrue );
