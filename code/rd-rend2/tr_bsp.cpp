...
static	void R_LoadLightmaps( lump_t *l, lump_t *surfs ) {
	...
	image = (byte *)Z_Malloc(tr.lightmapSize * tr.lightmapSize * 4 * 2, TAG_BSP, qtrue); // don't want to use TAG_BSP :< but it's the only one that fits
	...
	tr.lightmaps = (image_t **)Z_Malloc( tr.numLightmaps * sizeof(image_t*), TAG_HUNKALLOC, qtrue );

	if (tr.worldDeluxeMapping)
	{
		tr.deluxemaps = (image_t **)Z_Malloc( tr.numLightmaps * sizeof(image_t*), TAG_HUNKALLOC, qtrue );
	}
	...
	for(i = 0; i < numLightmaps; i++)
	{
		...
		// if (tr.worldLightmapping)
		{
			...
			lightScale = pow(2.0f, r_mapOverBrightBits->integer - tr.overbrightBits - 8); //exp2(r_mapOverBrightBits->integer - tr.overbrightBits - 8);
...
static	void R_LoadVisibility( lump_t *l ) {
	...
	s_worldData.novis = (byte*)Z_Malloc(len, TAG_HUNKALLOC, qfalse);
	...
	if ( tr.externalVisData ) {
		...
		dest = (byte *)Z_Malloc(len-8, TAG_HUNKALLOC, qfalse);
...
static void ParseFace( dsurface_t *ds, drawVert_t *verts, float *hdrVertColors, msurface_t *surf, int *indexes  ) {
	...
	cv->indexes = (glIndex_t *)Z_Malloc(numIndexes * sizeof(cv->indexes[0]), TAG_HUNKALLOC, qtrue);

	cv->numVerts = numVerts;
	cv->verts = (srfVert_t *)Z_Malloc(numVerts * sizeof(cv->verts[0]), TAG_HUNKALLOC, qtrue);
...
void R_MovePatchSurfacesToHunk(void) {
	...
	for ( i = 0; i < s_worldData.numsurfaces; i++ ) {
		...
		//
		size = sizeof(*grid);
		hunkgrid = (srfBspSurface_t *)Z_Malloc(size, TAG_HUNKALLOC, qfalse);
		Com_Memcpy(hunkgrid, grid, size);

		hunkgrid->widthLodError = (float*)Z_Malloc( grid->width * 4, TAG_HUNKALLOC, qfalse );
		Com_Memcpy( hunkgrid->widthLodError, grid->widthLodError, grid->width * 4 );

		hunkgrid->heightLodError = (float*)Z_Malloc( grid->height * 4, TAG_HUNKALLOC, qfalse );
		Com_Memcpy( hunkgrid->heightLodError, grid->heightLodError, grid->height * 4 );

		hunkgrid->numIndexes = grid->numIndexes;
		hunkgrid->indexes = (glIndex_t *)Z_Malloc(grid->numIndexes * sizeof(glIndex_t), TAG_HUNKALLOC, qfalse);
		Com_Memcpy(hunkgrid->indexes, grid->indexes, grid->numIndexes * sizeof(glIndex_t));

		hunkgrid->numVerts = grid->numVerts;
		hunkgrid->verts = (srfVert_t *)Z_Malloc(grid->numVerts * sizeof(srfVert_t), TAG_HUNKALLOC, qfalse);
...
static void R_CreateWorldVBOs(void)
{
	...
	// presort surfaces
	surfacesSorted = (msurface_t **)Z_Malloc(numSortedSurfaces * sizeof(*surfacesSorted), TAG_BSP, qtrue);
	...
	for(firstSurf = lastSurf = surfacesSorted; firstSurf < &surfacesSorted[numSortedSurfaces]; firstSurf = lastSurf)
	{
		...
		// create arrays
		verts = (srfVert_t *)Z_Malloc(numVerts * sizeof(srfVert_t), TAG_HUNKALLOC, qfalse); 
		indexes = (glIndex_t *)Z_Malloc(numIndexes * sizeof(glIndex_t), TAG_HUNKALLOC, qfalse); 
...
static	void R_LoadSurfaces( lump_t *surfs, lump_t *verts, lump_t *indexLump ) {
	...
	out = (msurface_t *)Z_Malloc( count * sizeof(*out), TAG_HUNKALLOC, qtrue );

	s_worldData.surfaces = out;
	s_worldData.numsurfaces = count;
	s_worldData.surfacesViewCount = (int *)Z_Malloc ( count * sizeof(*s_worldData.surfacesViewCount), TAG_HUNKALLOC, qtrue );
	s_worldData.surfacesDlightBits = (int *)Z_Malloc ( count * sizeof(*s_worldData.surfacesDlightBits), TAG_HUNKALLOC, qtrue );
	s_worldData.surfacesPshadowBits = (int *)Z_Malloc ( count * sizeof(*s_worldData.surfacesPshadowBits), TAG_HUNKALLOC, qtrue );
	...
	for ( i = 0 ; i < count ; i++, in++, out++ ) {
		switch ( LittleLong( in->surfaceType ) ) {
			...
			case MST_TRIANGLE_SOUP:
				out->data = (surfaceType_t *)Z_Malloc( sizeof(srfBspSurface_t), TAG_HUNKALLOC, qtrue );
				break;
			case MST_FLARE:
				out->data = (surfaceType_t *)Z_Malloc( sizeof(srfFlare_t), TAG_HUNKALLOC, qtrue );
				break;
			default:
				break;
		}
...
static	void R_LoadSubmodels( lump_t *l ) {
	...
	s_worldData.bmodels = out = (bmodel_t *)Z_Malloc( count * sizeof(*out), TAG_HUNKALLOC, qtrue );
...
static	void R_LoadNodesAndLeafs (lump_t *nodeLump, lump_t *leafLump) {
	...
	out = (mnode_t *)Z_Malloc ( (numNodes + numLeafs) * sizeof(*out), TAG_HUNKALLOC, qtrue );
...
static	void R_LoadShaders( lump_t *l ) {	
	...
	if (l->filelen % sizeof(*in))
		ri->Error (ERR_DROP, "LoadMap: funny lump size in %s",s_worldData.name);
	count = l->filelen / sizeof(*in);
	out = (dshader_t *)Z_Malloc( count*sizeof(*out), TAG_HUNKALLOC, qtrue );
...
{	
	...
	if (l->filelen % sizeof(*in))
		ri->Error (ERR_DROP, "LoadMap: funny lump size in %s",s_worldData.name);
	count = l->filelen / sizeof(*in);
	out = (int *)Z_Malloc( count*sizeof(*out), TAG_HUNKALLOC, qtrue );
...
static	void R_LoadPlanes( lump_t *l ) {
	...
	if (l->filelen % sizeof(*in))
		ri->Error (ERR_DROP, "LoadMap: funny lump size in %s",s_worldData.name);
	count = l->filelen / sizeof(*in);
	out = (cplane_t *)Z_Malloc ( count*2*sizeof(*out), TAG_HUNKALLOC, qtrue );
...
static	void R_LoadFogs( lump_t *l, lump_t *brushesLump, lump_t *sidesLump ) {
	...
	s_worldData.fogs = (fog_t *)Z_Malloc( s_worldData.numfogs*sizeof(*out), TAG_HUNKALLOC, qtrue );
...
void R_LoadLightGrid( lump_t *l ) {
	...
	w->lightGridData = (mgrid_t *)Z_Malloc( l->filelen, TAG_HUNKALLOC, qfalse );
	...
	if (r_hdr->integer)
	{
		...
		if (hdrLightGrid)
		{
			...
			w->hdrLightGrid = (float *)Z_Malloc(size, TAG_HUNKALLOC, qtrue );
...
void R_LoadLightGridArray( lump_t *l ) {
	...
	w->lightGridArray = (unsigned short *)Z_Malloc( l->filelen, TAG_BSP, qfalse );
...
void R_LoadEntities( lump_t *l ) {
	...
	w->entityString = (char *)Z_Malloc( l->filelen + 1, TAG_HUNKALLOC, qfalse );
...
static void R_LoadCubemapEntities(const char *cubemapEntityName)
{
	...
	tr.cubemapOrigins = (vec3_t *)Z_Malloc( tr.numCubemaps * sizeof(*tr.cubemapOrigins), TAG_HUNKALLOC, qtrue );
	tr.cubemaps = (image_t **)Z_Malloc( tr.numCubemaps * sizeof(*tr.cubemaps), TAG_HUNKALLOC, qtrue );
...
void R_MergeLeafSurfaces(void)
{
	...
	s_worldData.mergedSurfaces = (msurface_t *)Z_Malloc(sizeof(*s_worldData.mergedSurfaces) * numMergedSurfaces, TAG_HUNKALLOC, qtrue);
	s_worldData.mergedSurfacesViewCount = (int *)Z_Malloc(sizeof(*s_worldData.mergedSurfacesViewCount) * numMergedSurfaces, TAG_HUNKALLOC, qtrue);
	s_worldData.mergedSurfacesDlightBits = (int *)Z_Malloc(sizeof(*s_worldData.mergedSurfacesDlightBits) * numMergedSurfaces, TAG_HUNKALLOC, qtrue);
	s_worldData.mergedSurfacesPshadowBits = (int *)Z_Malloc(sizeof(*s_worldData.mergedSurfacesPshadowBits) * numMergedSurfaces, TAG_HUNKALLOC, qtrue);
	...
	s_worldData.viewSurfaces = (int *)Z_Malloc(sizeof(*s_worldData.viewSurfaces) * s_worldData.nummarksurfaces, TAG_HUNKALLOC, qtrue);
	...
	for (i = 0; i < numWorldSurfaces; i++)
	{
		...
		// create ibo
		ibo = tr.ibos[tr.numIBOs] = (IBO_t*)Z_Malloc(sizeof(*ibo), TAG_HUNKALLOC, qfalse);
		memset(ibo, 0, sizeof(*ibo));
		Q_strncpyz(ibo->name, va("staticWorldMesh_IBO_mergedSurfs%i", tr.numIBOs++), sizeof(ibo->name));
		numIboIndexes = 0;

		// allocate indexes
   		iboIndexes = outIboIndexes = (glIndex_t*)Z_Malloc(numIndexes * sizeof(*outIboIndexes), TAG_BSP, qtrue);
		...
		vboSurf = (srfBspSurface_t *)Z_Malloc(sizeof(*vboSurf), TAG_HUNKALLOC, qfalse);
...
void RE_LoadWorldMap( const char *name ) {
	...
	startMarker = (byte *)Z_Malloc(0, TAG_HUNKALLOC, qtrue);
	c_gridVerts = 0;
	...
	s_worldData.dataSize = (byte *)Z_Malloc(0, TAG_HUNKALLOC, qtrue) - startMarker;
