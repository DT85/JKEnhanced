...
qhandle_t RE_RegisterIndividualSkin( const char *name , qhandle_t hSkin) 
{
	...
	while ( text_p && *text_p ) {
		...
		surf = (skinSurface_t *) Z_Malloc( sizeof( *skin->surfaces[0] ), TAG_HUNKALLOC, qfalse );
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	tr.numSkins++;
	skin = (struct skin_s *)Z_Malloc( sizeof( skin_t ), TAG_HUNKALLOC, qfalse );
	tr.skins[hSkin] = skin;
...
void	R_InitSkins( void ) {
	...
	skin = tr.skins[0] = (struct skin_s *)Z_Malloc( sizeof( skin_t ), TAG_HUNKALLOC, qfalse );
	Q_strncpyz( skin->name, "<default skin>", sizeof( skin->name )  );
	skin->numSurfaces = 1;
	skin->surfaces[0] = (skinSurface_t *)Z_Malloc( sizeof( skinSurface_t ), TAG_HUNKALLOC, qfalse );
